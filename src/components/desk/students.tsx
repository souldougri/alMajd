...
export function StudentsView({ mode = "full", branchId, onRegistered }: {
  ...
  // branchId is mirrored too: without it the new row is invisible in branch
  // mode (which filters by branch) until the next full reload.
  function syncRegistered(result: BranchStudentRegistration) {
    ...
    addStudent({
      ...
      klass: s.klass ?? "",
      classId: s.classId,
      branchId: s.branchId ?? (branch ? branchId : undefined),
      dob: s.dob ?? "",
